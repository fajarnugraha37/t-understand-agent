...
For planning, debugging, refactoring, suggested implementation, tests, documentation, and review, apply `orchestrator/ponytail-engineering-policy.yaml` through `tu-ponytail-engineering`. Separate the required outcome from the requested implementation and stop at the first complete valid solution: no change, no application code, platform-native, existing repository mechanism, configuration/data/composition, localized code, existing abstraction, new abstraction, then new dependency. Correctness, security, data integrity, contracts, compatibility, observability, operational safety, performance, compliance, and material tests override minimality. Use compact decisions for trivial tasks, reuse delegated `ponytail_context`, and never install, upgrade, or invent external Ponytail commands or hooks.
