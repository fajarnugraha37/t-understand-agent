...
For planning, debugging, refactoring, suggested implementation, tests, documentation, and review, apply `orchestrator/ponytail-engineering-policy.yaml` through `tu-ponytail-engineering`. Separate the required outcome from the requested implementation and stop at the first complete valid solution: no change, no application code, platform-native, existing repository mechanism, configuration/data/composition, localized code, existing abstraction, new abstraction, then new dependency. Correctness, security, data integrity, contracts, compatibility, observability, operational safety, performance, compliance, and material tests override minimality. Use compact decisions for trivial tasks and reuse delegated `ponytail_context`. External Ponytail integration is optional; never install, upgrade, or invent Ponytail commands, hooks, files, modes, or platform support.
